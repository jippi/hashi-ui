    ...
    <div className="tab-pane active">
      <div className="row">
        <div className="col-md-6 tab-column">
          <legend>Tasks</legend>
          {tasks.length > 0
            ? <TableHelper classes="table table-hover table-striped" headers={taskHeaders} body={tasks} />
            : null}
        </div>
        <div className="col-md-6 tab-column">
          <legend>
            Task: {taskGroupId && taskId ? `${taskGroupId}/${taskId}` : null}
          </legend>
          {job.TaskGroups
            .filter(taskGroup => taskGroup.ID === taskGroupId)
            .map(taskGroup =>
              taskGroup.Tasks.filter(task => task.ID === taskId).map(task => <RawJson json={task} />).pop()
            )
            .pop()}
        </div>
      </div>

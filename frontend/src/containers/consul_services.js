...
class ConsulServices extends Component {
	...
	matchService(service, key, value) {
		let matched = false
		Object.keys(service).map(index => {
			if (index.toLowerCase() == key) {
				if (
					(value[0] == "~" && String(service[index]).match(value.slice(1))) ||
					String(service[index]) == value
				) {
					matched = true
				}
			}
				...
				<Row>
						...
						<Paper>
							<List style={listStyle}>
								{this.filteredServices().map(service => {
									...
									const style =
										this.props.routeParams.name == service.Name ? { backgroundColor: grey200 } : {}
					...
					<Col id="value-pane" key="value-pane" xs={12} sm={12} md={8} lg={8}>
						...
						{this.getConsulService().map((entry, index) => {
							if (this.props.routeParams.key && this.props.routeParams.value) {
								if (
									!this.matchService(
										entry.Service,
										this.props.routeParams.key,
										this.props.routeParams.value
									)
								) {
									return
								}
								...
								return (
									<Card key={`${check.Name}-${check.CheckID}-${check.Status}`}>
										...
										<CardText expandable>
											...
											<br />
											<div className="content-file small">{check.CheckID}</div>
							...
							return (
								...
								>
									<CardHeader
										title={`${entry.Node.Node} - ${entry.Service.ID}`}
